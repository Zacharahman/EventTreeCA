...
void powertfree(struct power *p)
{
  ...
  if (p->correlation_bother){
    tfree(p->autocorrelation);p->autocorrelation=NULL;
    tfree(p->crosscorrelation);p->crosscorrelation=NULL;}
...
void suite_7_power_process_helper(char *gs2)
{
  ...
  double maxdia=10000,xpos=0,ypos=0;
  int remove_flag=SUITE_7_CLEANUP,jpeg_flag=(ON_MY_COMPUTER ? 1 : 0),eps_flag=(ON_MY_COMPUTER ? 1 : 0);
  int raster_plot_flag=0,reliability_plot_flag=0,rank_plot_flag=1;
  ...
  if (!checktofind(dirname)){ printf(" %% warning! cannot find %s in suite_7_power_process_helper\n",dirname);}
	...
	if (reliability_plot_flag && nv==VARNAME_REGISTRY_wilson_spike_flag){
	  ...
	  for (nbicuculline=0;nbicuculline<total_bicucullines;nbicuculline++){ for (nodor=0;nodor<total_odors;nodor++){ for (nconcentration=0;nconcentration<total_concentrations;nconcentration++){
		...
		WritePNMfile_color(&(ra_reliability[0+0*total_bins+temp_tab*total_bins*total_records]),total_bins,total_records,temp_max,temp_min,filename,7);
		sprintf(filename,"%s.fig",filename_base);
		if ((fp=fopen(filename,"w"))==NULL){ printf(" %% warning! couldn't open %s in suite_7_power_process_helper\n",filename); fp=stdout;}
		fprintf(fp,"%s",FIG_PREAMBLE);
    ...
    if (rank_plot_flag){
      for (nbicuculline=0;nbicuculline<total_bicucullines;nbicuculline++){ for (nconcentration=0;nconcentration<total_concentrations;nconcentration++){
...
void system_monitor(struct neuronarray *Nra,double t,double DT)
{
  ...
  double *timera=NULL;
  double pulse_waitfor=0,pulse_howlong=0,pulse_temp=0;
...
/* 	      for (nr2=0;nr2<ninputs;nr2++){ tfree(fnamebase[nr2]);fnamebase[nr2]=NULL;} */
  ...
  case 7: 
    /* run multiple odors for multiple trials over various concentrations. 
       ...
       minimum(global_nclusters,nodors) combinatorially different odors, 
       minimum(5,nbicucullines) alternate architectures.
       nbicuculline==0: standard architecture
       nbicuculline==1: nAch blocked
       nbicuculline==2: gabaA blocked
       nbicuculline==3: gabaB blocked 
       nbicuculline==4: gabaA and gabaB blocked */
    SUITE_NSECONDS = maximum(2,SUITE_NSECONDS);
...
void snxdata_chain_0_snx(struct snxdata *s)
{
  ...
  switch (version_flag){
  case 0: printf(" %% warning! snxdata_chain_0_snx not designed for version %d\n",version_flag); break;
  case 1: if (verbose){ printf(" %% snxdata_chain_0_snx designed for version %d\n",version_flag);}
    ...
    if ((fp=fopen(filename,"w"))==NULL){ printf(" %% Can't create %s in snxdata_chain_0_snx\n",filename); fp=stdout;}
    fprintf(fp,";\n");
...
struct snxdata *snxdatamake(struct neuronarray *Nra,int lookback)
{
  ...
  for (nr1=0;nr1<s->length;nr1++){ s->stra[nr1] = strobemake(s->lookback+1,1,0,0,0,0);}
  ...
  case 3: /* two way ring */
    ...
    break;
  default: break;}
  raprintf(s->connectivity,"double",s->length,s->length,"s->connectivity: ");
  return s;
}
...
{
  int verbose=0;
  int nr=0,nl=0,nr2=0,tab=0,tab2=0;
  struct neuron *n=NULL;
  if (verbose){ printf(" %% [entering snxdataupdate] t %f dt %f\n",t,DT);}
  for (nr=0;nr<s->length;nr++){
    n=nget(s->Nra,0,nr);
    strobeupdate(s->stra[nr],t,DT,*(n->vpra[VARNAME_REGISTRY_snx_spike_flag]));
    if (verbose>2){ printf("nr %d tab %d\n",nr,s->stra[nr]->tab); raprintf(s->stra[nr]->data,"double",1,s->stra[nr]->length,"data: ");}}
  for (nr=0;nr<s->length;nr++){
