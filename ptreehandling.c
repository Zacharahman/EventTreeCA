...
{
  /* specially designed for yisun's data files */  
  int verbose=GLOBAL_verbose;
  int nr=0,zerovalue=0,length=GLOBAL_Lra_length,nspikes=0;
  double last_event = r->last_event;
  struct litem **lra=GLOBAL_lra;
  periodify("int",&(r->label),&zerovalue,&length,&nr);
  if (verbose){ printf(" %% [entering region_has_event] with region %d record %d...",r->label,nr);}
  if (lra[nr]!=NULL && *(double *)lra[nr]->item>=0){
    nspikes=0;
    while (lra[nr]!=NULL && *(double *)lra[nr]->item>=t && *(double *)lra[nr]->item<=t+DT){
      nspikes += 1;
      last_event = maximum(last_event,*(double *)lra[nr]->item);
      lra[nr]=lra[nr]->child;}
    if (nspikes>0){
      r->last_event = last_event;
      if (verbose){ printf(" %d spikes found, last_event %f\n",nspikes,r->last_event);}
      return 1;}}
  if (verbose){ printf(" no spikes found\n");}
  return 0;
}
